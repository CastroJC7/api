...
//mandar a llamar rutas
router.get('/api/materias-tecnico', (req, res) =>{
    res.setHeader('Content-Type', 'application/json');
    res.status(200).send(JSON.stringify(TecnicoIngenieriaComputacion, null, 2)); // El tercer argumento, 2, hace que la salida esté indentada con 2 espacios.
});

router.get('/api/materias-ingenieria', (req, res) =>{
// Convierte el objeto en una cadena JSON y establece el encabezado Content-Type
res.setHeader('Content-Type', 'application/json');
res.status(200).send(JSON.stringify(IngenieriaenCienciasdelaComputacion, null, 2)); // El tercer argumento, 2, hace que la salida esté indentada con 2 espacios.
});
